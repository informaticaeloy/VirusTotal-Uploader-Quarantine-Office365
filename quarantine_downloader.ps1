...
$ID = (Get-QuarantineMessage  -StartReceivedDate 07/04/2022 -EndReceivedDate 07/13/2022 | Where {$_.Type -eq 'Software Malicioso'}).Identity

$i = 0

foreach ($cadaID in $ID){ $ExportMessage = Export-QuarantineMessage -Identity $cadaID; $Encoding = [Convert]::FromBase64String($ExportMessage.Eml); [IO.File]::WriteAllBytes("C:\users\user\desktop\cuarentena\download\"+$i+".eml", $Encoding); $i ++}
